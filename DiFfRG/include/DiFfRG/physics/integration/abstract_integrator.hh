...
namespace DiFfRG
{
  namespace internal
  {
    ...
    std::array<size_t, dim> make_int_grid(const ConfigTree &json, const std::array<std::string, dim> &names)
    {
      std::array<size_t, dim> int_grid;
      for (int i = 0; i < dim; ++i)
        int_grid[i] = json.get_uint_or_warn("/integration/" + names[i], default_quadrature_order(names[i]));
