...
TEST_CASE("2D Buckley-Leverett Example 12 refines toward the fine-grid run", "[2d][FV][KT][buckley-leverett]")
{
  const auto fine = run_buckley_leverett(n_fine_cells, "buckley_leverett_2D_fine_grid");

  const auto downsampled_fine_coarse = average_to_grid(fine.final_values, n_fine_cells, n_coarse_cells);
  ...
  const auto downsampled_fine_medium = average_to_grid(fine.final_values, n_fine_cells, n_medium_cells);
  ...
  std::cout << std::scientific << std::setprecision(8)
            ...
            << ", max=" << medium.max << '\n'
            << "2D Buckley-Leverett 200x200 invariant check: mass_error=" << fine.final_mass - fine.initial_mass
            << ", min=" << fine.min << ", max=" << fine.max << '\n';
