...
namespace DiFfRG
{
  template <typename Regulator, int dim = 4> double optimize_x_extent(const ConfigTree &json)
  {
    ...
    const uint order = json.get_uint_or_warn("/integration/x_order", 32);
    const int verbosity = json.get_int("/output/verbosity", 0);
    const double x_extent_tolerance = json.get_double_or_warn("/integration/x_extent_tolerance", 1e-5);
