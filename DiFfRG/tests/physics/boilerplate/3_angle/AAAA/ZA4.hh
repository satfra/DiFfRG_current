...
namespace DiFfRG
{
  namespace Flows
  {
    template <template <typename, typename> class INT> class ZA4_integrator
    {
    public:
      ...
      {
        integrator = std::make_unique<INT<double, ZA4_kernel<__REGULATOR__>>>(
            quadrature_provider, grid_sizes, x_extent, json);
      }

      ZA4_integrator() = default;

      template <typename NT, typename... T> std::future<NT> request(T &&...t)
      {
        static_assert(std::is_same_v<NT, double>, "Unknown type requested of ZA4_integrator::request");
    ...
    private:
      ...
      std::unique_ptr<INT<double, ZA4_kernel<__REGULATOR__>>> integrator;
    };
  } // namespace Flows
} // namespace DiFfRG