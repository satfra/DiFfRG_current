...
namespace DiFfRG {
  ...
  class V_sigma_kernel
  {
    ...
    static KOKKOS_FORCEINLINE_FUNCTION auto kernel(const double& l1, const auto& k, const auto& N, const auto& T, const auto& m2Sigma)
    {
      using namespace DiFfRG;using namespace DiFfRG::compute;const auto _interp1 = RB(powr<2>(k), powr<2>(l1));
      const auto _interp2 = CothFiniteT(sqrt(powr<2>(l1) + m2Sigma + RB(powr<2>(k), powr<2>(l1))), T);
      const auto _interp3 = RBdot(powr<2>(k), powr<2>(l1));
      return 0.25 * _interp2 * _interp3 * sqrt(powr<-1>(_interp1 + powr<2>(l1) + m2Sigma));
    }

    static KOKKOS_FORCEINLINE_FUNCTION auto constant(const auto& k, const auto& N, const auto& T, const auto& m2Sigma)
    {
