                ...
                double hint_delta = -1.0, double tol_override = -1.0)
{
  ...
  if (hint_delta > 0) {
    ...
    if (lower_result && !upper_result) {
      ...
      log.info("Warm-start bracket orientation reversed (run(lower)=TRUE, run(upper)=FALSE); "
  ...
  if (hint_delta <= 0) {
    ...
    while (!bounds_ok && bound_iter++ < max_bound_iter) {
      ...
      if (run_at(lower_m2A)) {
        log.info("Lower-bound test succeeded at {:.12e}; tightening upper_m2A to this value", lower_m2A);
        upper_m2A = lower_m2A;
        upper_validated = true;
        if (bound_iter > step_grace) {
          step *= 2.0;
          log.info("Increasing lower-bound step to {:.12e}", step);
        }
        lower_m2A -= step;
      } else {
    ...
    if (!upper_validated) {
      ...
      while (!bounds_ok && bound_iter++ < max_bound_iter) {
        ...
        if (!run_at(upper_m2A)) {
          ...
          if (bound_iter > step_grace) {
            step *= 2.0;
            log.info("Increasing upper-bound step to {:.12e}", step);
          }
          upper_m2A += step;
          continue;
        } else {
      ...
      log.info("Upper bound already validated by lower-bound loop at {:.12e}", upper_m2A);
    }
  }
