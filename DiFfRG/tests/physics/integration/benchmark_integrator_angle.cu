...
TEST_CASE("Benchmark momentum integrals with 1 angle", "[4D integration][quadrature integration]")
{
  ...
  YangMillsFlowEquations<IntegratorAngleTBB> flows_TBB(json);
  flows_TBB.set_k(k);
  YangMillsFlowEquations<IntegratorAngleGPU> flows_GPU(json);
  flows_GPU.set_k(k);
  ...
  {
    meter.measure([&] {
      auto futures_ZA = request_data<double>(flows_GPU.ZA_integrator, grid1D, k, arguments);
      update_data(futures_ZA, &(dummy_data[0]));
    });
  };
  ...
  {
    meter.measure([&] {
      auto futures_ZA = request_data<double>(flows_TBB.ZA_integrator, grid1D, k, arguments);
      update_data(futures_ZA, &(dummy_data[0]));
    });
  };
}