...
TEST_CASE("Test 2d cartesian gpu momentum integrals", "[integration][quadrature integration]")
{
  const double qx_min = GENERATE(take(2, random(-2., -1.)));
  const double qx_max = GENERATE(take(2, random(1., 2.)));

  QuadratureProvider quadrature_provider;
  Integrator1DCartesianGPU<double, PolyIntegrand> integrator(quadrature_provider, {{32}}, 0., 0, qx_min, qx_max);
