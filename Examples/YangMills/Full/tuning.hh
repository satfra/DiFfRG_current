                ...
                double hint_m2A = 0.0, double hint_delta = -1.0, double tol_override = -1.0)
{
  ...
  if (hint_delta > 0) {
    ...
    if (lower_result && !upper_result) {
      // Reversed orientation: run(lower) = TRUE, run(upper) = FALSE — the
      // bracket straddles the root but with the predicate flipped relative to
      // bisection's convention (f(x_min)=FALSE, f(x_max)=TRUE). Swap so the
      // FALSE side ends up in x_min, even though x_min > x_max numerically.
      spdlog::get("m2Alog")->info("Warm-start bracket orientation reversed (run(lower)=TRUE, run(upper)=FALSE); "
                                  "swapping so the divergent side feeds x_min.");
      std::swap(lower_m2A, upper_m2A);
    } else if (lower_result == upper_result) {
  ...
  if (hint_delta <= 0) {
    ...
    // Bracket-finding step grows geometrically after a grace period so that a
    // far-off initial bracket can still be escaped within the iteration cap.
    const int max_bound_iter = 50;
    const int step_grace = 3;
    const double base_step = 0.1 * std::abs(init_m2A);
    bool upper_validated = false;

    // Lower-bound loop: we need run(lower_m2A) == false.
    // If it returns true, that value is simultaneously a valid (tighter)
    // upper bound, so record it before pushing lower_m2A further down.
    ...
    while (!bounds_ok && bound_iter++ < max_bound_iter) {
      ...
      if (run(json, "m2Alog")) {
        spdlog::get("m2Alog")->info("Lower-bound test succeeded at {:.12e}; tightening upper_m2A to this value",
                                    lower_m2A);
        upper_m2A = lower_m2A;
        upper_validated = true;
        if (bound_iter > step_grace) {
          step *= 2.0;
          spdlog::get("m2Alog")->info("Increasing lower-bound step to {:.12e}", step);
        }
        lower_m2A -= step;
      } else {
        // Lower bound is below the root, which is what bisection needs.
        bounds_ok = true;
      }
    }
    if (!bounds_ok)
    ...
    // Upper-bound loop: skip if the lower-bound loop already produced a value
    // where run() returned true (that's a valid upper bound by definition).
    if (!upper_validated) {
      bounds_ok = false;
      bound_iter = 0;
      step = base_step;
      while (!bounds_ok && bound_iter++ < max_bound_iter) {
        ...
        if (!run(json, "m2Alog")) {
          // Upper bound is below the root: that value becomes a valid
          // (tighter) lower bound, then we push upper_m2A further up.
          lower_m2A = upper_m2A;
          if (bound_iter > step_grace) {
            step *= 2.0;
            spdlog::get("m2Alog")->info("Increasing upper-bound step to {:.12e}", step);
          }
          upper_m2A += step;
          continue;
        } else {
      ...
      spdlog::get("m2Alog")->info("Upper bound already validated by lower-bound loop at {:.12e}", upper_m2A);
    }
  }
