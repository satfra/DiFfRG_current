...
TEST_CASE("Test 3D gpu momentum integrals", "[3D integration][quadrature integration]")
{
  constexpr int dim = 3;

  const double x_extent = GENERATE(take(1, random(1., 2.)));
  QuadratureProvider quadrature_provider;
  Integrator3DGPU<double, PolyIntegrand> integrator(quadrature_provider, {{64, 16, 16}}, x_extent);
