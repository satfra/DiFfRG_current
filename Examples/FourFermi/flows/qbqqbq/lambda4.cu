...
namespace DiFfRG
{
  namespace Flows
  {
    lambda4_integrator::lambda4_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 2> grid_sizes,
    ...
    void lambda4_integrator::set_T(const double value) { integrator->set_T(value); }
    void lambda4_integrator::set_q0_extent(const double value) { integrator->set_q0_extent(value); }
