...
TEMPLATE_TEST_CASE_SIG("Test adaptive QMC gpu momentum integrals", "[integration][QMC integration]", ((int dim), dim),
                       (1), (2), (3), (4))
{
  const double x_extent = GENERATE(take(1, random(1., 2.)));
  IntegratorQMC<dim, double, PolyIntegrand> integrator(x_extent, 1e-3, 1e-16, 128);
  ...
  SECTION("Volume integral")
  {
    ...
    if (!is_close(reference_integral, integral, dim == 1 ? 1e-2 : 1e-5)) {
    ...
    CHECK(is_close(reference_integral, integral, dim == 1 ? 1e-2 : 1e-5));
  }
