  ...
  meter.measure([&] {
    ...
    Kokkos::parallel_for(
        Kokkos::MDRangePolicy<Kokkos::Rank<5>>(
            space, {0, 0, 0, 0, 0},
            {integral_view.size(), n[0].size(), n[1].size(), n[2].size(), n[3].size()}), // range of the kernel
        KOKKOS_LAMBDA(const uint k, const uint idx0, const uint idx1, const uint idx2, const uint idx3) {
          // get the position for the current index
          const auto idx_v = coordinates.from_linear_index(k);
          ...
          const NT result =
              device::apply([&](const auto &...iargs) { return KERNEL::kernel(x0, x1, x2, x3, iargs...); }, full_args);
