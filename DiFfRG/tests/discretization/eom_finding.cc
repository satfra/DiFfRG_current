...
namespace
{
  ...
  JSONValue make_json(const int fe_order, const uint refinement = 0)
  {
    return json::value(
        {{"physical", {}},
         {"integration",
...
TEST_CASE("EoM configuration provides validated typed defaults", "[discretization][EoM][configuration]")
{
  ...
  const JSONValue empty_json = json::value({});
  ...
  const JSONValue custom_json = json::value({{"discretization",
                                              {{"EoM_abs_tol", 1e-9},
                                               {"EoM_max_iter", 17},
