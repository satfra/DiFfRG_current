...
namespace DiFfRG
{
  namespace Flows
  {
    lambda1_integrator::lambda1_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 2> grid_sizes,
    ...
    void lambda1_integrator::set_T(const double value) { integrator->set_T(value); }
    void lambda1_integrator::set_q0_extent(const double value) { integrator->set_q0_extent(value); }
