                       ...
                       (1), (2), (3), (4))
{
  ...
  SECTION("Random polynomials")
  {
    ...
    if (!is_close(reference_integral, integral_crosscheck, 1e-6)) {
    ...
    CHECK(is_close(reference_integral, integral_crosscheck, 1e-6));
  }
}
                       ...
                       ((int dim), dim), (1), (2), (3), (4))
{
  const double x_extent = GENERATE(take(1, random(1., 2.)));
  QuadratureProvider quadrature_provider;
  IntegratorTBB<dim, complex<double>, PolyIntegrand> integrator(quadrature_provider, 64, x_extent);
  ...
  SECTION("Volume integral")
  {
    ...
    const double integral = real(integrator.request(k, 0., 1., 0., 0., 0., 0., 0.).get());
