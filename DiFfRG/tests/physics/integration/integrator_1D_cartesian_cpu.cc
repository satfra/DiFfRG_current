...
TEST_CASE("Test 2d cartesian cpu momentum integrals (complex)", "[complex][cpu][integration][quadrature]")
{
  const double qx_min = GENERATE(take(2, random(-2., -1.)));
  const double qx_max = GENERATE(take(2, random(1., 2.)));

  QuadratureProvider quadrature_provider;
  Integrator1DCartesianTBB<complex<double>, PolyIntegrand> integrator(quadrature_provider, {{32}}, 0., 0, qx_min,
                                                                      qx_max);
...
TEST_CASE("Test 2d cartesian cpu momentum integrals (autodiff)", "[autodiff][cpu][integration][quadrature]")
{
  const double qx_min = GENERATE(take(2, random(-2., -1.)));
  const double qx_max = GENERATE(take(2, random(1., 2.)));

  QuadratureProvider quadrature_provider;
  Integrator1DCartesianTBB<autodiff::real, PolyIntegrand> integrator(quadrature_provider, {{32}}, 0., 0, qx_min,
                                                                     qx_max);
...
TEST_CASE("Test 2d cartesian cpu momentum integrals (complex autodiff)",
          "[complex][autodiff][cpu][integration][quadrature]")
{
  const double qx_min = GENERATE(take(2, random(-2., -1.)));
  const double qx_max = GENERATE(take(2, random(1., 2.)));

  QuadratureProvider quadrature_provider;
  Integrator1DCartesianTBB<cxReal, PolyIntegrand> integrator(quadrature_provider, {{32}}, 0., 0, qx_min, qx_max);
