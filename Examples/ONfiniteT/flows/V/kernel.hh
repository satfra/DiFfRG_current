...
static KOKKOS_FORCEINLINE_FUNCTION auto kernel(const double& l1, const T2& k, const T3& Nf, const T4& T, const auto& m2Pi, const auto& m2Sigma)
{
using namespace DiFfRG;using namespace DiFfRG::compute;const auto _interp1 = RB(powr<2>(k), powr<2>(l1));
const auto _interp2 = powr<-1>(tanh(0.5 * powr<-1>(T) * sqrt(powr<2>(l1) + m2Pi + RB(powr<2>(k), powr<2>(l1)))));
const auto _interp3 = RBdot(powr<2>(k), powr<2>(l1));
const auto _interp4 = powr<-1>(tanh(0.5 * powr<-1>(T) * sqrt(powr<2>(l1) + m2Sigma + RB(powr<2>(k), powr<2>(l1)))));
const auto _cse1 = powr<2>(l1);
  return 0.25 * fma(-1., _interp2 * _interp3 * sqrt(powr<-1>(_cse1 + _interp1 + m2Pi)), fma(_interp3, _interp4 * sqrt(powr<-1>(_cse1 + _interp1 + m2Sigma)), fma(_interp2, _interp3 * sqrt(powr<-1>(_cse1 + _interp1 + m2Pi)) * Nf, 0.)));
}

template<typename T1, typename T2, typename T3>
static KOKKOS_FORCEINLINE_FUNCTION auto constant(const T1& k, const T2& Nf, const T3& T, const auto& m2Pi, const auto& m2Sigma)
{
