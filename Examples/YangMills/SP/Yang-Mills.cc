 ...
 *
 * The output location is passed in rather than read from /output/folder, because the tuners give
 * every probe its own child directory; external_log routes this run's messages into the calling
 * tuner's log instead of the run's own session log.
 *
 ...
 * obs_out, because it is a perfectly good sample of the critical power law and discarding it is
 * pure waste. Both pointers may be null, and both outputs are NaN-initialised, so "wrote nothing"
         ...
         double *residual_out = nullptr)
{
  ...
  //            the safe side at Zc(0) = target. The gauge avatars plateau across the whole
  //            near-critical basin, so this reads the same couplings while landing on fast,
  //            non-divergent flows instead of resolving the razor separatrix -- which is what
  //            makes the nested STI search tractable.
...
int main(int argc, char *argv[])
{
  ...
  if (config.get_bool("/tuning/STI/tune")) {
    tune_STI(config, output_path, run);
  } else if (config.get_bool("/tuning/m2A/tune")) {
