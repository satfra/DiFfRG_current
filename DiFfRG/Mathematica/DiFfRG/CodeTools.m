...
MakeKernel[kernelExpr_, constExpr_, spec_Association, parameters_List,
   OptionsPattern[]] :=
  Module[{expr, const, kernel, constant, kernelClass, kernelHeader, integratorHeader,
     ...
     = OptionValue["Coordinates"], regulator, params, paramsAD, explParamAD,
     i, arguments, outputPath, sources, appendDefaultAssociation},
    ...
    kernelHeader = FunKit`MakeCppHeader["Includes" -> {"DiFfRG/physics/utils.hh"
       ...
       spec["Name"] <> "_kernel;"}];
    (*Print[kernelHeader];*)
    appendDefaultAssociation[config_] := Merge[{config, assoc2}, First
      ];
    params = FunKit`Private`prepParam /@ parameters;
    paramsAD = params;
    For[i = 1, i <= Length[params], i++,
      If[KeyFreeQ[params[[i]], "Type"],
        params[[i]] = Association[(Normal @ (params[[i]])) \[Union] {
          "Type" -> "double"}]
        ,
      ...
      If[KeyFreeQ[paramsAD[[i]], "Type"],
        paramsAD[[i]] = Association[(Normal @ (paramsAD[[i]])) \[Union]
           {"Type" -> "double"}]
        ,
    ...
    If[KeyExistsQ[spec, "Type"],
      AppendTo[integratorADTemplateParams, StringReplace[ToString[spec[
        "Type"]], $ADReplacementsDirect]]
      ,
        ...
        "Body" ->
          {
            ...
            FunKit`MakeCppClass[
              ...
              "MembersPublic" ->
                Join[
                  {
                    FunKit`MakeCppFunction["Name" -> spec["Name"] <> 
                      "_integrator", "Parameters" -> {<|"Type" -> "DiFfRG::QuadratureProvider",
                    ...
                    If[Length[coordinates] > 0,
                      FunKit`MakeCppFunction["Name" -> "map", "Templates"
                         -> {"NT=double"}, "Parameters" -> {tparams}, "Body" -> "static_assert(std::is_same_v<NT, double> || std::is_same_v<NT, autodiff::real>, \"Unknown type requested of "
...
if constexpr (std::is_same_v<NT, double>)
  ...
  map_AD(std::forward<decltype(t)>(t)...);",
                    ...
                    ,
                    getRegulator[OptionValue["Regulator"], OptionValue[
                      "RegulatorOpts"]]
                    ,
                    spec["Integrator"] <> "<" <> integratorTemplateParams
                       <> "> integrator;"
                    ,
                    If[spec["AD"],
                      spec["Integrator"] <> "<" <> integratorADTemplateParams
                         <> "> integrator_AD;"
                      ,
                      ""
                    ]
                  ...
                  ,
                  Map[FunKit`MakeCppFunction["Name" -> "map", "Return"
                     -> "void", "Body" -> None, "Parameters" -> Join[{<|"Name" -> "dest",
                     "Type" -> "double*", "Const" -> False|>, <|"Name" -> "coordinates", 
                    "Reference" -> True, "Type" -> #, "Const" -> True|>}, params]]&, coordinates
                    ]
                  ,
                  If[spec["AD"],
                      #
                      ,
                      {}
                    ]& @ Map[FunKit`MakeCppFunction["Name" -> "map", 
                      "Return" -> "void", "Body" -> None, "Parameters" -> Join[{<|"Name" ->
                       "dest", "Type" -> "autodiff::real*", "Const" -> False|>, <|"Name" ->
                       "coordinates", "Reference" -> True, "Type" -> #, "Const" -> True|>},
                       paramsAD]]&, coordinates]
                  ,
                  {
                    FunKit`MakeCppFunction["Name" -> "get", "Return" 
                      -> "void", "Body" -> None, "Parameters" -> Join[{<|"Name" -> "dest", 
                      "Type" -> "double", "Reference" -> True, "Const" -> False|>}, params]
                      ]
                    ,
                    If[spec["AD"],
                        #
                        ...
                        ""
                         ...
                         -> "void", "Body" -> None, "Parameters" -> Join[{<|"Name" -> "dest",
                         "Type" -> "autodiff::real", "Reference" -> True, "Const" -> False|>},
                         paramsAD]]
                  }
                ]
              ,
    ...
    integratorCpp["CT", "get"] = FunKit`MakeCppBlock["Includes" -> {"../"
       <> spec["Name"] <> ".hh"}, "Body" -> {FunKit`MakeCppFunction["Name" 
      -> "get", "Class" -> spec["Name"] <> "_integrator", "Body" -> "integrator.get(dest, "
       <> arguments <> ");", "Parameters" -> Join[{<|"Name" -> "dest", "Type"
       -> "double", "Reference" -> True, "Const" -> False|>}, params], "Return"
       -> "void"]}];
    integratorCpp["AD", "get"] = FunKit`MakeCppBlock["Includes" -> {"../"
       <> spec["Name"] <> ".hh"}, "Body" -> {FunKit`MakeCppFunction["Name" 
      -> "get", "Class" -> spec["Name"] <> "_integrator", "Body" -> "integrator_AD.get(dest, "
       <> arguments <> ");", "Parameters" -> Join[{<|"Name" -> "dest", "Type"
       -> "autodiff::real", "Reference" -> True, "Const" -> False|>}, paramsAD
      ], "Return" -> "void"]}];
...
Print[integratorCpp["AD","map"]];*)
    ...
    Do[
      AppendTo[sources, outputPath <> "src/CT_map_" <> ToString[i] <>
         ".cc"];
    ...
    If[spec["AD"],
      ...
      Do[
        AppendTo[sources, outputPath <> "src/AD_map_" <> ToString[i] 
          <> ".cc"];
