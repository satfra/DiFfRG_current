...
TEST_CASE("Test 2D cpu interpolation", "[2D][interpolation]")
{
  ...
  const float p2_stop = GENERATE(take(2, random(1, 100))) + p1_start;
  ...
  LinearInterpolator2D<float, Coordinates2D> interpolator(empty_data, coords);
