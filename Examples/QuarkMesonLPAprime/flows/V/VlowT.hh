...
namespace DiFfRG
{
  namespace Flows
  {
    class VlowT_integrator
    {
    public:
      VlowT_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 1> grid_sizes, const double x_extent, const JSONValue &json);
      VlowT_integrator(const VlowT_integrator &other);
      ~VlowT_integrator();

      template <typename NT, typename... T> std::future<NT> request(T &&...t)
      {
        static_assert(std::is_same_v<NT, double> || std::is_same_v<NT, autodiff::real>, "Unknown type requested of VlowT_integrator::request");
    ...
    private:
      ...
      std::unique_ptr<DiFfRG::IntegratorTBB<3, double, VlowT_kernel<__REGULATOR__>>> integrator;
      std::unique_ptr<DiFfRG::IntegratorTBB<3, autodiff::real, VlowT_kernel<__REGULATOR__>>> integrator_AD;
    };
  } // namespace Flows
} // namespace DiFfRG