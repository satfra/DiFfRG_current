                       ...
                       ((int dim), dim), (3), (4))
{
  ...
  const double q0_summands = 8;
  const double T = GENERATE(take(2, random(0.5, 1.)));
  const double k = GENERATE(take(2, random(0., 1.)));
  const double q0_extent = q0_summands * 10 * 2. * M_PI * T * GENERATE(take(1, random(1., 2.)));
  QuadratureProvider quadrature_provider;
  IntegratorAngleFiniteTq0GPU<dim, double, PolyIntegrand> integrator(quadrature_provider, {{64, 12, 12}}, x_extent,
  ...
  {
    IntegratorAngleFiniteTq0TBB<dim, double, PolyIntegrand> integrator_cpu(quadrature_provider, {{64, 12, 12}},
  ...
  /* TODO: Find a way to test this
    SECTION("Random polynomials")
    {
      ...
      integrator.request(k, constant, poly[0], poly[1], poly[2], poly[3], poly[4], poly[5],
      cos_poly[0], cos_poly[1], cos_poly[2], cos_poly[3],
      q0_poly[0], q0_poly[1], q0_poly[2], q0_poly[3]);
      const double integral = integrator.get();
