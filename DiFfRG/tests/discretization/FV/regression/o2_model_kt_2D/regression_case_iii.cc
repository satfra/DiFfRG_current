...
namespace
{
  ...
  class O2_Model_VI_B_III_Base : public def::AbstractModel<O2_Model_VI_B_III_Base<BoundaryPolicy>, Components>,
                                 public def::fRG,
                                 public def::LLFFlux<O2_Model_VI_B_III_Base<BoundaryPolicy>>,
                                 public def::FlowBoundaries<O2_Model_VI_B_III_Base<BoundaryPolicy>>,
                                 public BoundaryPolicy<O2_Model_VI_B_III_Base<BoundaryPolicy>>,
                                 public def::AD<O2_Model_VI_B_III_Base<BoundaryPolicy>>
  {
  public:
    O2_Model_VI_B_III_Base() : def::fRG(1.0) {}
    explicit O2_Model_VI_B_III_Base(const JSONValue &json) : def::fRG(json) {}
  ...
  using O2_Model_VI_B_III = O2_Model_VI_B_III_Base<def::FVDefaultBoundaries>;
  using O2_Model_VI_B_III_OriginCentered = O2_Model_VI_B_III_Base<def::OriginOddLinearExtrapolationBoundaries>;
  ...
  Gamma2 run_o2_vi_b_iii_gamma_regression(const GammaRunParameters &params = {})
  {
    ...
    O2Assembler<O2_Model_VI_B_III> assembler(discretization, model, json);
  ...
  Gamma2 run_o2_vi_b_iii_origin_centered_quarter_domain_regression()
  {
    kt_regression::ensure_logger();
    ...
    const JSONValue json = make_run_json(params, grid);
    O2_Model_VI_B_III_OriginCentered model(json);
    Mesh mesh(make_mesh_config(grid));
    Discretization discretization(mesh, json);
    O2Assembler<O2_Model_VI_B_III_OriginCentered> assembler(discretization, model, json);

    kt_regression::TemporaryDirectory tmp_dir("o2_model_kt_2D_case_iii_origin_centered_quarter_domain");
    std::clog << "[O2 DIAG] using temporary output directory " << tmp_dir.path << '\n';
    DataOutput<dim, VectorType> data_out(tmp_dir.path.string(),
                                         "o2_model_kt_2D_case_iii_origin_centered_quarter_domain", "output", json);
    auto adaptor = std::make_unique<NoAdaptivity<VectorType>>();
    ImplicitTimeStepper time_stepper(json, &assembler, &data_out, adaptor.get());
...
TEST_CASE("O2 Model VI_B_III origin-centered quarter-domain run extracts Gamma2 at the origin",
          "[2d][FV][KT][O2][VI_B_III][quarter-domain][slow]")
{
  const Gamma2 gamma = run_o2_vi_b_iii_origin_centered_quarter_domain_regression();
