...
namespace DiFfRG
{
  namespace FV
  {
    namespace KurganovTadmor
    {
      ...
      namespace internal
      {
        template <int dim> struct MeasureHelper {
        ...
        // template <typename NumberType, uint dim, uint n_components> struct Cache_Data {
        //   using VectorType = dealii::Vector<NumberType>;
        //   static constexpr uint n_faces = GeometryInfo<dim>::faces_per_cell;
        //   VectorType u;
        //   NumberType cell_area;
        //   std::array<Cache_Data *, n_faces> neighbors{nullptr};
        //   std::array<NumberType, n_faces> face_areas;
        //   Point<dim> cell_center;
        //   // Point<dim> cell_center;
        //   // std::array<Point<dim>, n_faces> face_centers;
        //   // std::array<dealii::Tensor<1, dim, NumberType>, n_components> u_grad;
        //   // std::array<std::array<dealii::Tensor<1, dim, NumberType>, n_components>, n_faces> u_n_grad;
        // };
      ...
      public:
                              ...
                              const VectorType & /* variables */ = VectorType()) override
        {
          ...
          const auto face_worker = [&](const Iterator &cell, const unsigned int &f, const unsigned int &sf,
            ...
            // const CacheData &cache = cache_data[cell->user_index()];
            ...
            std::array<Tensor<1, dim, ADNumberType>, n_components> F_AD_plus{}, F_AD_minus{};
            std::array<Tensor<1, dim, NumberType>, n_components> F_plus{}, F_minus{}, a_half{};
            ...
            for (size_t j = 0; j < n_components; ++j) {
              ...
              model.KurganovTadmor_advection_flux(F_AD_plus, x_q, internal::advection_flux_tie(u_plus_AD));
              model.KurganovTadmor_advection_flux(F_AD_minus, x_q, internal::advection_flux_tie(u_minus_AD));
            ...
            for (size_t d = 0; d < dim; ++d) {
              ...
              // Kurganov-Tadmor speed is the maximum of the local speeds
              NumberType a = std::max(max_eig_plus, max_eig_minus);

              // Broadcast the speed to all components (standard KT)
              for (size_t component : std::views::iota(0u, n_components)) {
                a_half[component][d] = a;
              }
            }
            ...
            for (uint dof = 0; dof < n_face_dofs; ++dof) {
              Tensor<1, dim, NumberType> numerical_flux{};
              ...
              Tensor<1, dim, NumberType> H_half =
                  (F_plus[component_i] + F_minus[component_i]) * 0.5 -
                  a_half[component_i] * (u_plus[component_i] - u_minus[component_i]) * 0.5;
              numerical_flux += H_half;
              copy_data_face.cell_residual(dof) +=
                  weight * JxW[q_face_index] * // dx
                  (fe_iv.jump_in_shape_values(dof, q_face_index, component_i) *
                   scalar_product(numerical_flux,
          ...
          const auto boundary_worker = [&](const Iterator &cell, const unsigned int &face_no, Scratch &scratch_data,
            ...
            // --- Kurganov-Tadmor numerical flux (same structure as face_worker) ---
            std::array<ADNumberType, n_components> u_plus_AD{}, u_minus_AD{};
            for (size_t i = 0; i < n_components; ++i) {
              u_plus_AD[i] = ADNumberType(u_plus[i]);
              u_minus_AD[i] = ADNumberType(u_minus[i]);
            ...
            for (uint dof = 0; dof < n_face_dofs; ++dof) {
              ...
              copy_data_face.cell_residual(dof) += weight * JxW[q_face_index] *
                                                   (fe_iv.jump_in_shape_values(dof, q_face_index, component_i) *
                                                    scalar_product(H_half, normals[q_face_index]));
            }
          };
                                   ...
                                   NumberType beta = 1.) override
        {
          ...
          //   auto &solution = scratch_data.solution_global;
          //   VectorType solution_dot(solution_global.size(), Vector<NumberType>(scratch_data.n_components));
          //   // auto &solution_dot = scratch_data.solution_dot;
          //   fe_v.get_function_values(solution_global, solution);
          //   fe_v.get_function_values(solution_global_dot, solution_dot);
