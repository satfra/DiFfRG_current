...
base_images=$(ls Base/)
# get all files in MPI+OpenMP/
mpi_openmp_images=$(ls MPI+OpenMP/)
# get all files in MPI+OpenMP+CUDA/
mpi_openmp_cuda_images=$(ls MPI+OpenMP+CUDA/)
mkdir -p logs

for image in $mpi_openmp_cuda_images; do
    echo "   Building DiFfRG docker image for $image..."
    docker buildx build -t diffrg-$image -f MPI+OpenMP+CUDA/$image . --no-cache --progress=plain &>logs/$image.log
    if [ $? -ne 0 ]; then
