...
#include "ZA4tadpole.kernel"

namespace DiFfRG
{
  namespace Flows
  {
    ZA4tadpole_integrator::ZA4tadpole_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 3> grid_sizes, const double x_extent, const ConfigTree &json)
    ...
    {
      integrator = std::make_unique<DiFfRG::Integrator4D2AngGPU<double, ZA4tadpole_kernel<__REGULATOR__>>>(quadrature_provider, grid_sizes, x_extent, json);
    }

    ZA4tadpole_integrator::ZA4tadpole_integrator(const ZA4tadpole_integrator &other)
        : quadrature_provider(other.quadrature_provider), grid_sizes(other.grid_sizes), jac_grid_sizes(other.jac_grid_sizes), x_extent(other.x_extent),
          ...
          integrator(
              std::make_unique<DiFfRG::Integrator4D2AngGPU<double, ZA4tadpole_kernel<__REGULATOR__>>>(other.quadrature_provider, other.grid_sizes, other.x_extent, other.json))
    {
    }

    ZA4tadpole_integrator::~ZA4tadpole_integrator() = default;

    std::future<double> ZA4tadpole_integrator::request_CT(
    ...
    double ZA4tadpole_integrator::get_CT(
        const double k, const double S0, const double S1, const double SPhi,
        const TexLinearInterpolator3D<double, CoordinatePackND<LogarithmicCoordinates1D<float>, LinearCoordinates1D<float>, LinearCoordinates1D<float>>> &ZA3,
        const TexLinearInterpolator3D<double, CoordinatePackND<LogarithmicCoordinates1D<float>, LinearCoordinates1D<float>, LinearCoordinates1D<float>>> &ZAcbc,
        const TexLinearInterpolator1D<double, LogarithmicCoordinates1D<float>> &ZA4SP,
