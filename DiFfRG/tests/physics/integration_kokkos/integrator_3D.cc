...
#include <DiFfRG/physics/integration_kokkos/integrator_3D.hh>
...
{
public:
  static KOKKOS_FORCEINLINE_FUNCTION auto kernel(const double qx, const double qy, const double qz, const double /*c*/,
...
TEST_CASE("Test 3D host integrals", "[double][cpu][integration][quadrature]")
{
  DiFfRG::Initialize();
  ...
  Integrator3D<double, PolyIntegrand, CPU_exec> integrator(quadrature_provider, {32, 16, 8}, {x_min, y_min, z_min},
                                                           {x_max, y_max, z_max});

  SECTION("Volume integral")
  {
    const double reference_integral = (x_max - x_min) * (y_max - y_min) * (z_max - z_min);
  ...
  SECTION("Random polynomials")
  {
    ...
    const double reference_integral =
        constant + x_poly.integral(x_min, x_max) * y_poly.integral(y_min, y_max) * z_poly.integral(z_min, z_max);

    double integral{};
    integrator.get(integral, constant, x_poly[0], x_poly[1], x_poly[2], x_poly[3], y_poly[0], y_poly[1], y_poly[2],
                   y_poly[3], z_poly[0], z_poly[1], z_poly[2], z_poly[3]);
    Kokkos::fence();
...
TEST_CASE("Test 3D device integrals", "[double][cpu][integration][quadrature]")
{
  DiFfRG::Initialize();
  ...
  QuadratureProvider quadrature_provider;
  Integrator3D<double, PolyIntegrand, GPU_exec> integrator(quadrature_provider, {32, 16, 8}, {x_min, y_min, z_min},
                                                           {x_max, y_max, z_max});
