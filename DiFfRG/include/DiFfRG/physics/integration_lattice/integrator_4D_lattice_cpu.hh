...
{
  /**
   * @brief Integration of an arbitrary 4D function from (qx_min, qy_min) to (qx_max, qy_max) using TBB.
   *
   * @tparam NT The numerical type of the result.
   * @tparam KERNEL The kernel to integrate.
   */
  template <typename NT, typename KERNEL> class Integrator4DCartesianTBB
  {
  public:
    /**
    ...
    /**
     * @brief Construct a new Integrator4DCartesianTBB object
     *
     ...
     */
    Integrator4DCartesianTBB(QuadratureProvider &quadrature_provider, const std::array<uint, 4> grid_sizes,
                             const ctype x_extent, const JSONValue &)
        : Integrator4DCartesianTBB(quadrature_provider, grid_sizes)
    {
    }
     ...
     */
    Integrator4DCartesianTBB(QuadratureProvider &quadrature_provider, std::array<uint, 4> grid_sizes,
                   ...
                   for (uint idx_x = r.pages().begin(); idx_x != r.pages().end(); ++idx_x) {
                     for (uint idx_y = r.rows().begin(); idx_y != r.rows().end(); ++idx_y) {
                       for (uint idx_z = r.cols().begin(); idx_z != r.cols().end(); ++idx_z) {
                         for (uint idx_a = 0; idx_a < grid_sizes[3]; ++idx_a) {
                           value += int_element * KERNEL::kernel(idx_x, idx_y, idx_z, idx_a, k, t...);
                         }
                       }
                     }
                   }
