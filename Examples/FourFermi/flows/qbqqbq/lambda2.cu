...
namespace DiFfRG
{
  namespace Flows
  {
    lambda2_integrator::lambda2_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 2> grid_sizes,
    ...
    void lambda2_integrator::set_T(const double value) { integrator->set_T(value); }
    void lambda2_integrator::set_q0_extent(const double value) { integrator->set_q0_extent(value); }
