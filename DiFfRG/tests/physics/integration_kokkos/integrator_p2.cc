...
#include <DiFfRG/physics/integration_kokkos/integrator_p2.hh>
...
public:
  static KOKKOS_INLINE_FUNCTION auto kernel(const double q, const double /*c*/, const double x0, const double x1,
                                            const double x2, const double x3, const double x4, const double x5)
  {
...
TEMPLATE_TEST_CASE_SIG("Test 1D momentum integrals on host", "[double][cpu][integration][quadrature]", ((int dim), dim),
                       (1), (2), (3), (4), (5))
{
  DiFfRG::Initialize();
  ...
  Integrator_p2<dim, double, PolyIntegrand, CPU_exec> integrator(quadrature_provider, {size}, x_extent);
  ...
  SECTION("Volume integral")
  {
    ...
    double integral{};
    integrator.get(integral, 0., 1., 0., 0., 0., 0., 0.);
    Kokkos::fence();
  ...
  SECTION("Random polynomials")
  {
    ...
    double integral{};
    integrator.get(integral, constant, x_poly[0], x_poly[1], x_poly[2], x_poly[3], x_poly[4], x_poly[5]);
    Kokkos::fence();
...
TEMPLATE_TEST_CASE_SIG("Test 1D momentum integrals on device", "[double][gpu][integration][quadrature]",
                       ((int dim), dim), (1), (2), (3), (4), (5))
{
  DiFfRG::Initialize();
  ...
  QuadratureProvider quadrature_provider;
  Integrator_p2<dim, double, PolyIntegrand, GPU_exec> integrator(quadrature_provider, {size}, x_extent);
  ...
  SECTION("Random polynomials")
  {
    ...
    const double expected_precision = max(1e-14, 2e-1 / pow((double)size, 2));
    const double rel_err = abs((integral - reference_integral) / reference_integral);
    if (rel_err >= expected_precision) {
      std::cerr << "reference: " << std::scientific << std::setw(10) << reference_integral
                << " | integral: " << std::setw(10) << integral << " | relative error: " << std::setw(12) << rel_err
