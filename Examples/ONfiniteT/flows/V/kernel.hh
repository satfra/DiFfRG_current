...
namespace DiFfRG {
  ...
  class V_kernel
  {
    ...
    static KOKKOS_FORCEINLINE_FUNCTION auto kernel(const double& l1, const auto& k, const auto& N, const auto& T, const auto& m2Pi, const auto& m2Sigma)
    {
      ...
      return 0.25 * fma(-1., _interp2 * _interp3 * sqrt(powr<-1>(_interp1 + powr<2>(l1) + m2Pi)), fma(_interp3, _interp4 * sqrt(powr<-1>(_interp1 + powr<2>(l1) + m2Sigma)), fma(_interp2, _interp3 * sqrt(powr<-1>(_interp1 + powr<2>(l1) + m2Pi)) * N, 0.)));
    }
