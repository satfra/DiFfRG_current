...
namespace DiFfRG
{
  namespace Flows
  {
    ZAcbc_integrator::ZAcbc_integrator(QuadratureProvider &quadrature_provider, std::array<uint, 3> grid_sizes, const double x_extent, const JSONValue &json)
    ...
    {
      integrator = std::make_unique<DiFfRG::Integrator4D2AngGPU<double, ZAcbc_kernel<__REGULATOR__>>>(quadrature_provider, grid_sizes, x_extent, json);
    }
    ...
    ZAcbc_integrator::ZAcbc_integrator(const ZAcbc_integrator &other)
        : quadrature_provider(other.quadrature_provider), grid_sizes(other.grid_sizes), jac_grid_sizes(other.jac_grid_sizes), x_extent(other.x_extent),
          ...
          integrator(std::make_unique<DiFfRG::Integrator4D2AngGPU<double, ZAcbc_kernel<__REGULATOR__>>>(other.quadrature_provider, other.grid_sizes, other.x_extent, other.json))
    {
    }

    ZAcbc_integrator::~ZAcbc_integrator() = default;

    std::future<double> ZAcbc_integrator::request_CT(const double k, const double p, const TexLinearInterpolator1D<double, LogarithmicCoordinates1D<float>> &ZA3,
    ...
    double ZAcbc_integrator::get_CT(const double k, const double p, const TexLinearInterpolator1D<double, LogarithmicCoordinates1D<float>> &ZA3,
                                    const TexLinearInterpolator1D<double, LogarithmicCoordinates1D<float>> &ZAcbc,
                                    const TexLinearInterpolator1D<double, LogarithmicCoordinates1D<float>> &ZA4,
                                    const TexLinearInterpolator1D<double, LogarithmicCoordinates1D<float>> &dtZc,
                                    const TexLinearInterpolator1D<double, LogarithmicCoordinates1D<float>> &Zc,
