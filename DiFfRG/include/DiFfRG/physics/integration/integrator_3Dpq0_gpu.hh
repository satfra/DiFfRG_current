...
namespace DiFfRG
{
  ...
  __global__ void gridreduce_3dpq0(NT *dest, const ctype *x_quadrature_p, const ctype *x_quadrature_w,
                                   ...
                                   const ctype q0_extent, const ctype k, T... t)
  {
    ...
    const ctype q0 = q0_quadrature_p[idx_z % len_y] * q0_extent;
    const ctype weight = q0_quadrature_w[idx_z % len_y] * q0_extent * 2 * (ctype)M_PI *
  ...
  template <typename NT, typename KERNEL> class Integrator3Dpq0GPU
  {
  public:
    using ctype = typename get_type::ctype<NT>;

    Integrator3Dpq0GPU(QuadratureProvider &quadrature_provider, const std::array<uint, 4> grid_sizes,
                       const ctype x_extent, const ctype q0_extent, const JSONValue &json)
        : Integrator3Dpq0GPU(quadrature_provider, grid_sizes, x_extent, q0_extent,
                             json.get_uint("/integration/cudathreadsperblock"))
    {
    }

    Integrator3Dpq0GPU(QuadratureProvider &quadrature_provider, std::array<uint, 4> grid_sizes, const ctype x_extent,
        ...
        : grid_sizes(grid_sizes), device_data_size(grid_sizes[0] * grid_sizes[1] * grid_sizes[2] * grid_sizes[3]),
          x_extent(x_extent), q0_extent(q0_extent),
          pool(rmm::mr::get_current_device_resource(), (device_data_size / 256 + 1) * 256)
    {
      ...
      ptr_q0_quadrature_p = quadrature_provider.get_device_points<ctype>(grid_sizes[3]);
      ptr_q0_quadrature_w = quadrature_provider.get_device_weights<ctype>(grid_sizes[3]);
    ...
    template <typename... T> NT get(const ctype k, const T &...t) const
    {
      ...
      gridreduce_3dpq0<ctype, NT, KERNEL><<<num_blocks, threads_per_block, 0, cuda_stream.value()>>>(
...
#include <DiFfRG/physics/integration/integrator_3Dpq0_cpu.hh>

namespace DiFfRG
{
  template <typename NT, typename KERNEL> class Integrator3Dpq0GPU : public Integrator3Dpq0TBB<NT, KERNEL>
  {
  public:
    using ctype = typename get_type::ctype<NT>;
    ...
    Integrator3Dpq0GPU(QuadratureProvider &quadrature_provider, std::array<uint, 4> grid_sizes, const ctype x_extent,
                       const ctype q0_extent, const uint max_block_size = 256)
        : Integrator3Dpq0TBB<NT, KERNEL>(quadrature_provider, grid_sizes, x_extent, q0_extent)
    {
    }
